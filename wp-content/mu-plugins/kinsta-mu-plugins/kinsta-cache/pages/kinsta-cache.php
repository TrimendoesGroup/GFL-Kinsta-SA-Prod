<?php namespace Kinsta ?>
<div class="wrap">

    <div class='kinsta-page-bar'>
        <?php if( KINSTAMU_WHITELABEL === false ) : ?>
            <img class='logo' src='<?php echo plugin_dir_url( __FILE__ ) ?>../../shared/images/logo-dark.svg' height='16px'>
        <?php elseif( defined('KINSTAMU_LOGO') ) : ?>
            <img class='logo' src='<?php echo KINSTAMU_LOGO ?>' height='32px'>
        <?php endif ?>
        <h3><?php _e( 'Cache Control', 'kinsta-cache' ) ?></h3>
    </div>

    <div class='kinsta-page-wrapper'>

        <div class='kinsta-main-content'>

            <div class='kinsta-box'>

                <div class='kinsta-box-title-bar'>
                    <h3><?php _e( 'Cache Settings', 'kinsta-cache' ) ?></h3>
                </div>

                <div class='kinsta-box-content'>
                    <div class='content mb22'>
                        <p><?php _e( 'Your site uses our full page and object caching technology to remain lightning fast. We recommend leaving all options on their default settings for optimal performance.', 'kinsta-cache') ?>
                        </p>
                    </div>

                    <div class='' style='border-left:6px solid #D95454; background: #FB8686; margin-bottom:22px; color: #fff; padding:22px;'><?php _e( 'The default options below are finely tuned for your servers. Please only change them if you know what you\'re doing otherwise they may actually slow down your site! Note that third party caching systems such as WP Rocket are disabled as they can slow down your site when used in conjunction with your server-level caching.', 'kinsta-cache' ) ?></div>

                    <?php include( 'partials/cache-settings-form.php') ?>

                </div>
            </div>

        </div>

        <div class='kinsta-sidebar'>

            <?php if( $this->KinstaCache->KinstaCachePurge->has_object_cache ) : ?>
                <?php include( 'partials/sidebar-purge-has-object-cache.php' ) ?>
            <?php else : ?>
                <?php include( 'partials/sidebar-purge-no-object-cache.php' ) ?>
            <?php endif ?>

            <?php
                if( KINSTAMU_WHITELABEL === false ) {
                    include( 'partials/sidebar-support.php' );
                }
            ?>
        </div>

    </div>

</div>


<script>
jQuery(document).on('click', '.kinsta-clear-cache', function() {
    var element = jQuery(this);
    jQuery.ajax({
        url: '<?php echo admin_url('admin-ajax.php') ?>',
        type: 'post',
        data: {
            kinsta_nonce: element.attr('data-nonce'),
            action: element.attr( 'data-action' )
        }
    })
    return false;
})
</script>
